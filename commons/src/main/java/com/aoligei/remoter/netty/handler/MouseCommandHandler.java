 ...
 * 2020-9-4
 * 点对点模式服务端-鼠标输入处理器
 * 业务上：鼠标输入的信息是由Master端发出的，服务端并不负责键盘输入的解析工作，仅仅
 * 负责将该鼠标输入信息转发给与之对应的Slave端。需注意的是，尽管在代码中是遍历
 * targetClientId分别转发该消息；但因目前暂不支持单个Master同时关注多个Slaver，
 * 所以实质上仍是点对点的业务场景。这部分的控制在RequestInspect的相关内容中。
 */
public class MouseCommandHandler extends AbstractServerCensorC2CHandler{
    ...
    protected void particularHandle(ChannelHandlerContext channelHandlerContext, BaseRequest baseRequest) throws NettyServerException {
         ...
         * 转发消息给受控客户端
         */
        BaseResponse baseResponse = BuildUtil.buildResponse(baseRequest.getClientId(),baseRequest.getTargetClientIds(), CommandEnum.MOUSE_INPUT,baseRequest.getData(),null);
